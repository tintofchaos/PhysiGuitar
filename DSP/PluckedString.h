...
/* replace function with a macro */
#define SINE_PROCESS(sine, x) \
    x = sine.wavetable[(int)sine.wavetable_pos]; \
    if (floorf(sine.wavetable_pos) != ceilf(sine.wavetable_pos)) \
        x = (sine.wavetable_pos - floorf(sine.wavetable_pos) ) * sine.wavetable[(int)ceilf(sine.wavetable_pos)] + (ceilf(sine.wavetable_pos) - sine.wavetable_pos) * sine.wavetable[(int)floorf(sine.wavetable_pos)]; \
    sine.wavetable_pos += sine.freq / ( (float) sine.sample_rate / (float) WAVETABLE_SIZE ); \
...
void string_noteon(PluckedString *string, float f0, float velocity) {
    ...
    for (int i = 0; i < MAX_MODES_AMOUNT; i++) {
        ...
        if (f0 != 0) {
            n = (float) i + 1.f;
            freq_coeff = sqrtf(1.f + (string->stiffness * string->stiffness) * (n * n) );
...
void string_noteoff(PluckedString *string) {
    string->noteoff = 1;
    for (int i = 0; i < MAX_MODES_AMOUNT; i++) {
        sine_reset(&string->modes[i]);
        sine_reset(&string->coupling_modes[i]);
    }
}
...
float string_process(PluckedString *string) {
    ...
    float n, freq, detune_freq;
    float decay_time, coupled_decay_time;
    ...
    for (int i = 0; i < MAX_MODES_AMOUNT; i++) {
        ...
        if (freq >= (float) string->sample_rate / 2.f || freq >= 20000) // stop at 20,000Khz
            break;
            
        decay_time = string->damping + string->muting * freq * M_PI * 2.f;

	    float sine, coupled_sine;

	    SINE_PROCESS(string->modes[i], sine)

        sum += sine * string->amplitudes[i] * expf(-string->t * decay_time) / 12.f;
	    ...
	    SINE_PROCESS(string->coupling_modes[i], coupled_sine)
	
        sum += coupled_sine * string->amplitudes[i] * expf(-string->t * coupled_decay_time) / 12.f;
    }

...
#define STRING_NYLON 0x0
#define STRING_STEEL 0x1

// youngs modulus of strings
#define STELL_STIFFNESS 0.1570795
#define NYLON_STIFFNESS 0.0043196

#define PITCHSHIFT_DELAY 0.04 // 40 ms delay buffer for the pitchshifter
...
void string_setmaterial(PluckedString *string, int material) {
...
void string_update(PluckedString *string) {
    if (!string->updated) {
        ...
        for (int i = 0; i < MAX_MODES_AMOUNT; i++) {
            ...
            if (string->material == STRING_STEEL)
                stiffness = STELL_STIFFNESS / tension * 9.86960440109;
            else if (string->material == STRING_NYLON)
                stiffness = NYLON_STIFFNESS / tension * 9.86960440109;

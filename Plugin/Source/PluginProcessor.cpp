...
void PhysiGuitarAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < 6; i++) {
        strings[i]->stiffness = *stiffness;
        strings[i]->position = *pluck_position;
        
        if (*pluck_position != prev_position) strings[i]->updated = 0;
        
        strings[i]->damping = *damping;
        strings[i]->muting = *muting * 0.005;

...
{
    addParameter(material = new juce::AudioParameterChoice({"material", 1}, "String Material", juce::StringArray { "Nylon", "Steel"}, 1) );
...
void PhysiGuitarAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if ((*material).getCurrentChoiceName() == "Nylon")
            mat = 0;
        else if ((*material).getCurrentChoiceName() == "Steel")
            mat = 1;
...
void PhysiGuitarAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    material->setValueNotifyingHost(stream.readInt() );
